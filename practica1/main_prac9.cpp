...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	t_roof.LoadTGA("roof_tiles.tga");
	t_roof.BuildGLTexture();
	t_roof.ReleaseImage();

	t_window.LoadTGA("cabin_window.tga");
	t_window.BuildGLTexture();
	t_window.ReleaseImage();

	t_roofside.LoadTGA("roof_side.tga");
	t_roofside.BuildGLTexture();
	t_roofside.ReleaseImage();

	t_front.LoadTGA("cabin_front.tga");
	t_front.BuildGLTexture();
	t_front.ReleaseImage();
}
...
void triangular(GLuint textura1, GLuint textura2) {
	GLfloat vertice[8][3] = {
		{ -0.5 ,-sqrt(3) / 6, 0.5 },    //Coordenadas Vértice 0 V0
		{ 0.5 , -sqrt(3) / 6, 0.5 },    //Coordenadas Vértice 1 V1
		{ 0.0 , 1 / sqrt(3), 0.5 },    //Coordenadas Vértice 2 V2
		{ -0.5 ,-sqrt(3) / 6, -0.5 },    //Coordenadas Vértice 3 V3
		{ 0.0 , 1 / sqrt(3), -0.5 },    //Coordenadas Vértice 4 V4
		{ 0.5 , -sqrt(3) / 6, -0.5 },    //Coordenadas Vértice 5 V5
	};
		glBindTexture(GL_TEXTURE_2D, textura1);   // choose the texture to use.
		glBegin(GL_POLYGON);	//Front
			//glColor3f(1.0,1.0,1.0);
			...
			glTexCoord2f(1.0f, 2.0f); glVertex3fv(vertice[2]);
		glEnd();
	
		glBegin(GL_POLYGON);	//Back
			glNormal3f( 0.0f, 0.0f, -1.0f);
			glTexCoord2f(2.0f, 0.0f); glVertex3fv(vertice[3]);
			glTexCoord2f(1.0f, 2.0f); glVertex3fv(vertice[4]);
			glTexCoord2f(0.0f, 0.0f); glVertex3fv(vertice[5]);
		glEnd();

		glBindTexture(GL_TEXTURE_2D, textura2);
		glBegin(GL_POLYGON);	//Left
			glNormal3f( -0.86f, 0.5f, 0.0f);
			glTexCoord2f(1.0f, 0.0f); glVertex3fv(vertice[0]);
			glTexCoord2f(1.0f, 1.0f); glVertex3fv(vertice[2]);
			glTexCoord2f(0.0f, 1.0f); glVertex3fv(vertice[4]);
			glTexCoord2f(0.0f, 0.0f); glVertex3fv(vertice[3]);
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(pos_camX, pos_camY, pos_camZ);
		...
		glPushMatrix(); 
			...
			prisma(t_front.GLindex, t_window.GLindex, t_metal01.GLindex);
		glPopMatrix();
		glDisable(GL_ALPHA_TEST);

		glPushMatrix();
			glTranslatef(0.0, 1.51 + (2.0 * (sqrt(3)/6)) , 0.0);
			glScalef(4.0, 2.0, 4.0);
			triangular(t_roofside.GLindex, t_roof.GLindex);
		glPopMatrix();
